...
def create_survey(user_id, syndrome_id)
  survey = Survey.new(
    user_id: user_id, 
...
def create_syndromes(user_id, syndrome_id)
  syndrome = Survey.new(
    threshold_score: 2.1, 
  )
  return syndrome
...
RSpec.describe Group, type: :model do
  ...
  it "test create group" do
    group = create_group('salve');
    group.save()
    expect(group.description).to eq("salve")
  end

  it "test create survey" do
    sydnrome = create_syndromes();
    user = create_user();
    user.save();
    syndrome.save()
    survey = create_survey();
    survey.save();
    id = survey.symptom_id;
    expect(id).to eq("teste")
  end
