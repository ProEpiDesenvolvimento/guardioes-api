...
class UsersController < ApplicationController
  ...
  before_action :authenticate_user!, except: %i[index panel_list show update destroy create query_by_param email_reset_password reset_password show_reset_token admin_update]
  before_action :authenticate_group_manager!, only: [:group_data]
  before_action :set_user_update, only: %i[update admin_update]
  ...
  def email_reset_password
    ...
    UserMailer.reset_password_email(@user).deliver if @user.present?
  ...
  def panel_list
    @current_user = if current_user.nil? && current_manager.nil? && current_group_manager.nil?
                      current_admin
  ...
  def to_csv
    attributes = []
    @users.first.search_data.each do |key, _value|
    ...
    CSV.generate(headers: true) do |csv|
      csv << attributes
      @users.each do |user|
        csv << user.search_data.map { |_key, value| value.to_s }
      end
    end
  end
