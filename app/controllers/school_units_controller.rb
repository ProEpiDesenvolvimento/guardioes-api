...
class SchoolUnitsController < ApplicationController
  before_action :set_school_unit, only: %i[show update destroy]
  ...
  def filtering
    ...
    when ->(h) { !h[:state].nil? }
      SchoolUnit.where(state: h[:state])
    when ->(h) { h[:category] == 'SES-DF' && !h[:level].empty? && !h[:city].empty? }
      SchoolUnit.where(category: h[:category], level: h[:level], city: h[:city])
    when ->(h) { h[:category] == 'UNB' || h[:category] == 'IFB' }
      SchoolUnit.where(category: h[:category])
    else
      SchoolUnit.all
    end
  end
