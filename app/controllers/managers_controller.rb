...
class ManagersController < ApplicationController
  ...
  def index
    return render json: @app.managers, status: :ok if @app.managers.empty?
  ...
  def update
    if @manager.update(manager_params.except(:permissions))
      ...
      if params['manager'].key?(:permissions)
  ...
  def email_reset_password
    ...
    ManagerMailer.reset_password_email(@manager).deliver if @manager.present?
